...
void main() {
  ...
  test('bounded browse keeps best rows when they arrive last', () async {
    final worker = SearchWorker();
    addTearDown(worker.close);

    final base = DateTime.utc(2027, 1, 1);
    final records = List<Medicine>.generate(
      260,
      (index) => Medicine(
        id: 'reverse-$index',
        name: 'Medicine $index',
        expiry: base.add(Duration(days: 259 - index)),
        quantity: 10,
      ),
      growable: false,
    );
  ...
  test('bounded removed browse keeps newest rows when they arrive last', () async {
    final worker = SearchWorker();
    addTearDown(worker.close);
    ...
    final first = await worker.browseArchived(records, 1, limit: 5);
    expect(
      first.map((hit) => hit.id),
      [
        'removed-reverse-259',
